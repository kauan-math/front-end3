...
const CORES_CHART = ["#10b981", "#f59e0b", "#3b82f6", "#8b5cf6", "#ec4899"];
...
function AlertCard({
  ...
  return (
    <div className={`rounded-xl border p-4 ${baseColor} flex gap-3`}>
...
export default function AdminDashboardPage() {
  ...
  const [searchFuncionarios] = useState("");
  ...
  const [configForm, setConfigForm] = useState<Configuracao>({
    unidade: "GymFlow",
  ...
  async function carregarDados() {
    try {
      ...
      const [resAlunos, resFuncionarios, resReceitas, resDespesas] =
        await Promise.all([
  ...
  async function adicionarFuncionario(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        ...
        email: `${novoFuncionario.nome
          .toLowerCase()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .replace(/\s+/g, ".")}@gymflow.com`,
  ...
  async function salvarConfig(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      localStorage.setItem("config", JSON.stringify(configForm));
  ...
  async function marcarComoPago(receita: Receita) {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/receitas/${receita.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const funcionariosFiltrados = useMemo(() => {
    return funcionarios.filter((func) =>
      func.nome.toLowerCase().includes(searchFuncionarios.toLowerCase()),
    );
  ...
  const receitaPorPlano = useMemo(() => {
    const agrupar = alunos.reduce(
      (acc, aluno) => {
        ...
        const existente = acc.find((r) => r.plano === plano);
    ...
    const total = agrupar.reduce((sum, r) => sum + r.valor, 0);

    return agrupar.map((r) => ({
      ...r,
      porcentagem: total > 0 ? (r.valor / total) * 100 : 0,
    }));
  }, [alunos]);
  ...
  const totalReceita = useMemo(() => {
    return receitas.reduce((sum, r) => sum + numero(r.valorPagamento), 0);
  }, [receitas]);
  ...
  const taxaOcupacao = useMemo(() => {
    return configForm.capacidadeMaxima
      ? Math.round(
          (alunos.length / parseInt(configForm.capacidadeMaxima)) * 100,
        )
      : 0;
  }, [alunos, configForm]);
  ...
  async function adicionarDespesa(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          mensagem = erroApi?.error || erroApi?.message || mensagem;
        } catch {
      ...
      alert(err instanceof Error ? err.message : "Erro ao cadastrar despesa.");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-zinc-950 via-black to-zinc-950 p-4 sm:p-6 lg:p-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <StatCard
                ...
                trend={alunos.length > 10 ? "up" : "down"}
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              ...
              >
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={receitasChartData}>
                    <XAxis
                      dataKey="mes"
                      stroke="#71717a"
                      style={{
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => formatarTooltip(value)}
                    />
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <PieChart>
                      <Pie
                        ...
                        outerRadius={80}
                      >
                        {receitaPorPlano.map((_, index) => (
                          <Cell
                            key={index}
                            fill={CORES_CHART[index % CORES_CHART.length]}
                          />
                        ))}
                      </Pie>

                      <Tooltip formatter={(value) => formatarTooltip(value)} />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 bg-zinc-900/50 rounded-lg">
                    <span className="text-sm text-zinc-400">Novos alunos</span>

                    <span className="text-lg font-bold text-emerald-400">
                      +{Math.floor(alunos.length * 0.2)}
                    </span>
                  </div>

                  <div className="flex items-center justify-between p-3 bg-zinc-900/50 rounded-lg">
                    <span className="text-sm text-zinc-400">Cancelamentos</span>
                  ...
                  <div className="flex items-center justify-between p-3 bg-zinc-900/50 rounded-lg">
                    <span className="text-sm text-zinc-400">Satisfação</span>
              ...
              >
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {alunosFiltrados.length === 0 ? (
                    ...
                    alunosFiltrados.slice(0, 8).map((aluno) => (
                      <div
                        key={aluno.id}
                    ...
                    <tbody className="divide-y divide-zinc-900 font-mono">
                      {despesasFiltradas.length === 0 ? (
                        <tr>
                          <td
                            colSpan={8}
                            className="px-4 py-6 text-center text-zinc-500 font-sans"
                          >
                        ...
                        despesasFiltradas.map((despesa) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 font-sans">
                              <span
                                className={`px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wider ${categoriaColor(despesa.categoria)}`}
                              >
                                {despesa.categoria}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3 text-zinc-400">
                              {despesa.dataPagamento
                                ? formatarData(despesa.dataPagamento)
                                : "-"}
                            </td>
                            <td className="px-4 py-3 font-sans">
                              <span
                              ...
                              >
                                {despesa.status ||
                                  (despesa.dataPagamento ? "PAGO" : "PENDENTE")}
                              </span>
                            </td>
                            {isAdm && (
                                ...
                                <div className="flex items-center justify-end gap-2">
                                  {!despesa.dataPagamento &&
                                    despesa.status?.toUpperCase() !==
                                      ...
                                      <button
                                        onClick={() =>
                                          marcarDespesaComoPaga(despesa.id)
                                        }
            ...
            <div className="mt-6">
              <SectionCard icon={History} title="Sistema">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-zinc-900/30 rounded-lg border border-zinc-800/50">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">Sincronizado</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 p-4 bg-zinc-900/30 rounded-lg border border-zinc-800/50">
                    ...
                    <div>
                      <p className="text-sm font-medium text-white">Servidor</p>

                      <p className="text-xs text-zinc-500">Operacional</p>
                    </div>
                  </div>
                </div>
              </SectionCard>
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-zinc-500 mt-4">Carregando dados...</p>
            </div>
          </div>
        )}
      </div>
